...
func (_q *UserQuery) QueryStat() *UserStatQuery {
	query := (&UserStatClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithStat(opts ...func(*UserStatQuery)) *UserQuery {
	query := (&UserStatClient{config: _q.config}).Query()
...
func (_q *UserQuery) loadStat(ctx context.Context, query *UserStatQuery, nodes []*User, init func(*User), assign func(*User, *UserStat)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userstat.FieldUserID)
	}
